...
# Formats each edited .xaml with the pinned XAML Styler tool and the committed per-project
# reference styles, via .claude/hooks/Format-Xaml.ps1 (single source of truth, which selects
# the PSADT style for src/** and the Fluence style for lib/Fluence.Wpf/**, and enforces the
# owning project's BOM/line-ending policy). Non-blocking: formatting issues never fail the
# tool call; run .claude/hooks/Format-Xaml.ps1 -Check as the hard gate.
...
foreach ($raw in $paths) {
    ...
    try {
        # Use the same host the hook runs under; Format-Xaml.ps1 is 5.1-compatible, so this
        # avoids a hard dependency on pwsh 7.
