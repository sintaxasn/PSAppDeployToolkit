...
- **Setting only `Window.Background` transparent for a DWM backdrop, leaving the HWND redirection surface black** -> a top-level WPF window has two background layers, and `HwndTarget.BackgroundColor` (the redirection surface WPF clears behind the content) defaults to opaque black. With an active backdrop the content `Background` is transparent, so the default-black redirection surface flashes before the system backdrop composites (the first-paint "black flash"), and it is worst under forced software rendering where the gap is widest. Fix: in `ApplyBackdrop`, set `HwndSource.CompositionTarget.BackgroundColor` to the same color as `Window.Background` (transparent for an active backdrop, the opaque theme fallback for `None`). This eliminates the redirection-surface race but does not cover the pre-render mapping gap (see the first-paint hold pitfall below).
- **First-paint black-slide under forced software rendering** -> even with the redirection-surface fix and the glass-frame gate, the HWND is mapped (visible in the OS) several display frames before WPF produces its first CPU-rendered frame. During that gap DWM shows the window's default (black) surface at its default unpositioned location; `PositionWindow` and `SizeToContent` then move and grow it while still black. Hardware rendering hides this because WPF's first frame is near-instant; `SoftwareOnly` makes the gap multi-frame and visible. Fix: `FluenceWindow.HideForFirstPaint` (called in `OnSourceInitialized` after `ApplyWindowShell`) keeps the HWND invisible during the gap; `RevealAfterFirstPaint` shows it once the software render has SETTLED, not merely once `OnContentRendered` has fired: under `SoftwareOnly` the visual tree rasterizes progressively over several composition frames after `OnContentRendered` (frame capture showed only a subset of content present at `OnContentRendered`, the rest filling in over ~8 frames), so revealing on the first tick uncovers a half-painted dialog. The reveal is deferred to `DefaultFirstPaintRevealTicks` (14) `CompositionTarget.Rendering` ticks past `OnContentRendered`, then a `DwmFlush()` to land the final present; this self-corrects across dialog kinds -- animated dialogs keep ticking past their content render, static dialogs stop ticking exactly when their render completes (so the safety timer reveals them already-complete). Gate: `ShouldGuardFirstPaint()` = `ProcessRenderMode == SoftwareOnly || !IsCompositionEnabled()` -- hardware + composition path is excluded. Preferred mechanism is `DWMWA_CLOAK` (composites Mica behind the cloaked window so the backdrop is ready at reveal); fallback is `WS_EX_LAYERED` + alpha 0 when composition is unavailable. Safety: 1000 ms `DispatcherTimer` ensures the window is never permanently hidden. Do NOT reveal on the FIRST tick (too early -- partial content under software rendering); do NOT keep the hold on the hardware-composited path. The reveal timing is tunable for field diagnosis via the dormant `FLUENCE_FP_REVEAL` env var (`flush` / `ticks<N>` / `ticks<N>flush`; default 14 + flush), and the first-paint lifecycle can be traced via `FLUENCE_FP_TRACE=1` (both are no-ops unless set).
- **Content-entrance discipline: hold containers hidden from their first realization** -> gating the pre-animation hidden state on `IsLoaded` means the initial item batch renders at full opacity before the list is shown; a later `CollectionChanged` Reset then re-prepares those containers at `Opacity = 0` and fades them in, producing a flash-vanish-fade artifact. Fix: `PrepareContainerForItemOverride` unconditionally sets `container.Opacity = 0` (and starts a `TranslateTransform` offset) for every animated container, regardless of `IsLoaded`. If the list is already loaded, `BeginItemEntranceAnimation` runs immediately; otherwise `OnListViewLoaded` fades the initial batch once. Rule: first content should appear instantly (already opaque from load), not flash-at-full-opacity-then-animate. See `Fluence.Wpf/Controls/ListView.cs` for the canonical implementation and `ControlTests.ListView.cs` for the `ListViewPrepareProbe` regression test that pins the invariant.
- **Going transparent for a backdrop that DWM will not actually composite** -> the redirection-surface fix above only helps when DWM is genuinely painting Mica/Acrylic behind the transparent client. When it is not -- DWM composition disabled, or the user's "Transparency effects" turned off (`HKCU\...\Themes\Personalize\EnableTransparency = 0`) -- a transparent window has nothing behind it: it flashes the uncomposited surface on first paint and stays see-through while transparency is off. Gating the backdrop on OS *version* alone (`OsVersionHelper`) misses both conditions. Fix: `WindowCapabilities.BackdropCompositionAvailable` is a runtime gate (`DwmIsCompositionEnabled` AND `RegistryHelper.IsTransparencyEnabled` AND a non-synthetic primary display adapter), and `WindowPolicy.ResolveEffectiveBackdrop` downgrades the effective backdrop to `None` (opaque theme background for both `Window.Background` and the redirection surface) whenever it is false. An opaque base cannot flash; this is the INKORE / WPF-UI parity behavior for non-composited sessions. Note: `RenderOptions.ProcessRenderMode` (WPF software rasterization) is deliberately NOT part of this gate -- DWM desktop composition is independent of WPF's render mode, so Mica renders correctly under forced software rendering as long as DWM composition and transparency are on. A virtualized desktop needs the third condition: a Hyper-V VM's synthetic `Microsoft Hyper-V Video` adapter reports `RenderCapability.Tier` = 2 yet DWM still cannot composite Mica on it, so the transparent client bled the uncomposited accent as an L-band. `NativeMethods.IsPrimaryDisplayAdapterMicrosoftBasic` (`EnumDisplayDevices` -> primary adapter name starts with `Microsoft`) catches it: Microsoft ships no real GPUs, so any Microsoft-vendor display adapter (Hyper-V Video, Basic Display Adapter, Basic Render Driver, Remote Display Adapter) is synthetic and forces the opaque `None` path; real GPUs and Hyper-V GPU-PV / passthrough expose their true vendor and are unaffected. Do NOT use `RenderCapability.Tier` as the discriminator -- it survives forced `SoftwareOnly` and reads 2 on the VM.
