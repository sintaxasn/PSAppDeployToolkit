...
- **First-paint black-slide under forced software rendering** -> even with the redirection-surface fix and the glass-frame gate, the HWND is mapped (visible in the OS) several display frames before WPF produces its first CPU-rendered frame. During that gap DWM shows the window's default (black) surface at its default unpositioned location; `PositionWindow` and `SizeToContent` then move and grow it while still black. Hardware rendering hides this because WPF's first frame is near-instant; `SoftwareOnly` makes the gap multi-frame and visible. Fix: `FluenceWindow.HideForFirstPaint` (called in `OnSourceInitialized` after `ApplyWindowShell`) keeps the HWND invisible during the gap; `RevealAfterFirstPaint` (deferred to the first `CompositionTarget.Rendering` tick after `OnContentRendered`) shows it once WPF has finished painting, the on-screen position is committed, and DWM has had one frame to composite. Gate: `ShouldGuardFirstPaint()` = `ProcessRenderMode == SoftwareOnly || !IsCompositionEnabled()` -- hardware + composition path is excluded. Preferred mechanism is `DWMWA_CLOAK` (composites Mica behind the cloaked window so the backdrop is ready at reveal); fallback is `WS_EX_LAYERED` + alpha 0 when composition is unavailable. Safety: 1000 ms `DispatcherTimer` ensures the window is never permanently hidden. Do NOT reveal on an eager N-render-tick counter. Do NOT keep the hold on the hardware-composited path.
